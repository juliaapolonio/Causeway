...
```{r, echo=FALSE, eval = !isFALSE(params$report_logo)}
htmltools::includeCSS(params$css)
```

```{r results="asis", eval = !isFALSE(params$report_logo), echo=FALSE}
cat(paste0("
<style>
#TOC {
...
report_subtitle <- paste0('juliaapolonio/PROSTAGMA ', pipeline_version)
```
...
```{r, eval = !isFALSE(params$forest_plot), out.width='100%', dpi=1200, fig.align='center'}
knitr::include_graphics(params$forest_plot)
```

```{r, eval = !isFALSE(params$volcano_plot), out.width='100%', dpi=1200, fig.align='center'}
knitr::include_graphics(params$volcano_plot)
```
...
final_results <- final_results[final_results$is_candidate == TRUE, c("gene", "nsnp", "b_Inverse_variance_weighted", "pval_Inverse_variance_weighted", "gsmr_beta", "gsmr_pval", "causal_snp")]
